 ...
 * <p>
 * {@link Sweepable} resources may be added to or removed from a {@link Sweeper} and the resource
 * implementation decides whether it should be swept or not.</p>
 * <p>
 * If a {@link Sweepable} resources is itself a container of other sweepable resources, it will forward the
 * sweep operation to children resources, and so on recursively.</p>
 * <p>
 * Typical usage is to add {@link Sweeper} as a bean to an existing container:</p>
 * <pre>
 * Server server = new Server();
 * server.addBean(new Sweeper(), true);
 * server.start();
 * </pre> Code that knows it has sweepable resources can then lookup the {@link Sweeper} and offer the
 * sweepable resources to it:
 * <pre>
...
public class Sweeper extends AbstractLifeCycle implements Runnable
{
    ...
    /**
     * <p>
     * A {@link Sweepable} resource implements this interface to signal to a {@link Sweeper} or to a parent
     * container if it needs to be swept or not.</p>
     * <p>
     * Typical implementations will check their own internal state and return true or false from
     * {@link #sweep()} to indicate whether they should be swept.</p>
     */
