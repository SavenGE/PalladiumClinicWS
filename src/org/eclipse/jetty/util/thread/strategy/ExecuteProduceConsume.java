...
/**
 * <p>
 * A strategy where the thread calls produce will always run the resulting task itself. The strategy may
 * dispatches another thread to continue production.
 * </p>
 * <p>
 * The strategy is also known by the nickname 'eat what you kill', which comes from the hunting ethic that
 * says a person should not kill anything he or she does not plan on eating. In this case, the phrase is used
 * to mean that a thread should not produce a task that it does not intend to run. By making producers run the
 * task that they have just produced avoids execution delays and avoids parallel slow down by running the task
 * in the same core, with good chances of having a hot CPU cache. It also avoids the creation of a queue of
 * produced tasks that the system does not yet have capacity to consume, which can save memory and exert back
