...
/**
 * <p>
 * A callback wrapper that succeeds the wrapped callback when the count is reached, or on first failure.</p>
 * <p>
 * This callback is particularly useful when an async operation is split into multiple parts, for example when
 * an original byte buffer that needs to be written, along with a callback, is split into multiple byte
 * buffers, since it allows the original callback to be wrapped and notified only when the last part has been
 * processed.</p>
 * <p>
 * Example:</p>
 * <pre>
