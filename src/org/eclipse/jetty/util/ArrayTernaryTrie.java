 ...
 * <p>
 * This Trie is of a fixed size and cannot grow (which can be a good thing with regards to DOS when used as a
 * cache).
 * </p>
 * <p>
 ...
 * </p>
 * <dl>
 * <dt>char[] _tree</dt><dd>This is semantically 2 dimensional array flattened into a 1 dimensional char
 * array. The second dimension is that every 4 sequential elements represents a row of: character; hi index;
 * eq index; low index, used to build a ternary trie of key strings.</dd>
 * <dt>String[] _key</dt><dd>An array of key values where each element matches a row in the _tree array. A non
 * zero key element indicates that the _tree row is a complete key rather than an intermediate character of a
 * longer key.</dd>
 * <dt>V[] _value</dt><dd>An array of values corresponding to the _key array</dd>
 * </dl>
 * <p>
 * The lookup of a value will iterate through the _tree array matching characters. If the equal tree branch is
 * followed, then the _key array is looked up to see if this is a complete match. If a match is found then the
...
public class ArrayTernaryTrie<V> extends AbstractTrie<V>
{
     ...
     * This is actually a 2 dimensional array that has been flattened to achieve locality of reference.
     */
     ...
     *
     * @param capacity The capacity of the Trie, which is in the worst case is the total number of characters
     * of all keys stored in the Trie. The capacity needed is dependent of the shared prefixes of the keys.
     * For example, a capacity of 6 nodes is required to store keys "foo" and "bar", but a capacity of only 4
     * is required to store "bar" and "bat".
     */
