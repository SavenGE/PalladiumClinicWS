...
public class PathWatcher extends AbstractLifeCycle implements Runnable
{
    public static class Config
    {
        ...
        /**
         * Add a <code>glob:</code> syntax pattern exclude reference in a directory relative, os neutral,
         * pattern.
         *
         * <pre>
         *    On Linux:
         ...
         *
         * @param dir the directory to build new Config from (using this config as source of
         * includes/excludes)
         ...
         * <p>
         * -999 indicates arbitrarily deep recursion, 0 indicates no recursion, 1 is only one directory deep,
         * and so on.
         *
         ...
         *
         * @param child the child directory to test against
     ...
     * PathPendingEvents
     *
     * For a given path, a list of events that are awaiting the quiet time. The list is in the order that the
     * event were received from the WatchService
     */
    public static class PathPendingEvents
    {
         ...
         * <p>
         * Will validate the timestamp to see if it is expired, as well as if the file size hasn't changed
         ...
         *
         * @param expiredDuration the expired duration past the timestamp to be considered expired
         * @param expiredUnit the unit of time for the expired check
    ...
    /**
     * Request watch on a the given path (either file or dir) using all Config defaults. In the case of a dir,
    ...
    /**
     * Register path in the config with the file watch service, walking the tree if it happens to be a
     * directory.
     *
    ...
    /**
     * Check to see if the watcher is in a state where it should generate watch events to the listeners. Used
     * to determine if watcher should generate events for existing files and dirs on startup.
     *
     ...
     *
     * When an event for a path first occurs, it is subjected to a quiet time. Subsequent events that arrive
     * for the same path during this quiet time are accumulated and the timer reset. Only when the quiet time
     * has expired are the accumulated events sent. MODIFY events are handled slightly differently - multiple
     * MODIFY events arriving within a quiet time are coalesced into a single MODIFY event. Both the
     * accumulation of events and coalescing of MODIFY events reduce the number and frequency of event
    ...
    /**
     * Add an event reported by the WatchService to list of pending events that will be sent after their quiet
     * time has expired.
     *
    ...
    /**
     * Whether or not to issue notifications for directories and files that already exist when the watcher
     * starts.
     *
