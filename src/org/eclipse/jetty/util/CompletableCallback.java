...
/**
 * <p>
 * A callback to be used by driver code that needs to know whether the callback has been succeeded or failed
 * (that is, completed) just after the asynchronous operation or not, typically because further processing
 * depends on the callback being completed. The driver code competes with the asynchronous operation to
 * complete the callback.
 * </p>
 * <p>
 * If the callback is already completed, the driver code continues the processing, otherwise it suspends it.
 * If it is suspended, the callback will be completed some time later, and {@link #resume()} or
 * {@link #abort(Throwable)} will be called to allow the application to resume the processing.
 * </p>
 * Typical usage:
 * <pre>
...
public abstract class CompletableCallback implements Callback
{
     ...
     * <em>after</em> the asynchronous operation to detect whether the asynchronous operation has already
     * completed or not.
     *
