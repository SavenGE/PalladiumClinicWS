...
/**
 * Date Format Cache. Computes String representations of Dates and caches the results so that subsequent
 * requests within the same second will be fast.
 *
 * Only format strings that contain either "ss". Sub second formatting is not handled.
 *
 * The timezone of the date may be included as an ID with the "zzz" format string or as an offset with the
 * "ZZZ" format string.
 *
 * If consecutive calls are frequently very different, then this may be a little slower than a normal
 * DateFormat.
 */
public class DateCache
{
 ...
 /* ------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Constructor. Make a DateCache that will use a default format. The default format generates the same
     * results as Date.toString().
     */
    public DateCache()
    {
        this(DEFAULT_FORMAT);
    ...
    /* ------------------------------------------------------------ */
    /**
     * Format a date according to our stored formatter. If it happens to be in the same second as the last
    ...
    /* ------------------------------------------------------------ */
    /**
     * Format a date according to our stored formatter. The passed time is expected to be close to the current
     * time, so it is compared to the last value passed and if it is within the same second, the format is
     * reused. Otherwise a new cached format is created.
     *
