 ...
 * <p>
 * The standard JVM {@link ByteBuffer} can exist in two modes: In fill mode the valid data is between 0 and
 * pos; In flush mode the valid data is between the pos and the limit. The various ByteBuffer methods assume a
 * mode and some of them will switch or enforce a mode: Allocate and clear set fill mode; flip and compact
 * switch modes; read and write assume fill and flush modes. This duality can result in confusing code such
 * as:
 * </p>
 * <pre>
 *     buffer.clear();
 ...
 * </p>
 * <p>
 * The BufferUtil class provides a set of utilities that operate on the convention that ByteBuffers will
 * always be left, passed in an API or returned from a method in the flush mode - ie with valid data between
 * the pos and limit. This convention is adopted so as to avoid confusion as to what state a buffer is in and
 * to avoid excessive copying of data that can result with the usage of compress.</p>
 * <p>
 * Thus this class provides alternate implementations of {@link #allocate(int)},
 * {@link #allocateDirect(int)} and {@link #clear(ByteBuffer)} that leave the buffer in flush mode. Thus the
 * following tests will pass:
 * </p>
 * <pre>
 ...
 * {@link #append(ByteBuffer, byte[], int, int)} or {@link #put(ByteBuffer, ByteBuffer)} are used, then the
 * caller does not need to explicitly switch the buffer to fill mode. If the caller wishes to use other
 * ByteBuffer bases libraries to fill a buffer, then they can use explicit calls of #flipToFill(ByteBuffer)
 * and #flipToFlush(ByteBuffer, int) to change modes. Note because this convention attempts to avoid the
 * copies of compact, the position is not set to zero on each fill cycle and so its value must be remembered:
 * </p>
 * <pre>
 ...
 * </pre>
 * <p>
 * The flipToFill method will effectively clear the buffer if it is empty and will compact the buffer if there
 * is no space.
 * </p>
 */
public class BufferUtil
{
    ...
    /* ------------------------------------------------------------ */
    /**
     * Allocate ByteBuffer in flush mode. The position and limit will both be zero, indicating that the buffer
     * is empty and must be flipped before any data is put to it.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * Clear the buffer to be empty in fill mode. The position is set to 0 and the limit is set to the
     * capacity.
     *
     * @param buffer The buffer to clear.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Flip the buffer to fill mode. The position is set to the first unused position in the buffer (the old
     * limit) and the limit is set to the capacity. If the buffer is empty, then this call is effectively
     * {@link #clearToFill(ByteBuffer)}. If there is no unused space to fill, a {@link ByteBuffer#compact()}
     * is done to attempt to create space.
     * <p>
     * This method is used as a replacement to {@link ByteBuffer#compact()}.
     *
     * @param buffer The buffer to flip
     * @return The position of the valid data before the flipped position. This value should be passed to a
     * subsequent call to {@link #flipToFlush(ByteBuffer, int)}
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Flip the buffer to Flush mode. The limit is set to the first unused byte(the old position) and the
     ...
     * @param position The position of valid data to flip to. This should be the return value of the previous
     * call to {@link #flipToFill(ByteBuffer)}
     */
     ...
     *
     * @param from Buffer to take bytes from in flush mode
     * @param to Buffer to put bytes to in flush mode. The buffer is flipToFill before the put and flipToFlush
     * after.
     * @return number of bytes moved
    ...
    /**
     * Convert buffer to an long. Parses up to the first non-numeric character. If no number is found an
     * IllegalArgumentException is thrown
     *
     * @param buffer A buffer containing an integer in flush mode. The position is not changed.
     * @return an int
     ...
     * @return A string showing the escaped content of the buffer around the position and limit (marked with
     * &lt;&lt;&lt; and &gt;&gt;&gt;)
     */
