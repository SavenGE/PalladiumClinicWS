 ...
 * <p>
 * This implementation is always case insensitive and is optimal for a small number of fixed strings with few
 * special characters. The Trie is stored in an array of lookup tables, each indexed by the next character of
 * the key. Frequently used characters are directly indexed in each lookup table, whilst infrequently used
 * characters must use a big character table.
 * </p>
 * <p>
 * This Trie is very space efficient if the key characters are from ' ', '+', '-', ':', ';', '.', 'A' to 'Z'
 * or 'a' to 'z'. Other ISO-8859-1 characters can be used by the key, but less space efficiently.
 * </p>
 * <p>
 * This Trie is not Threadsafe and contains no mutual exclusion or deliberate memory barriers. It is intended
 * for an ArrayTrie to be built by a single thread and then used concurrently by multiple threads and not
 * mutated during that access. If concurrent mutations of the Trie is required external locks need to be
 * applied.
 * </p>
 *
...
public class ArrayTrie<V> extends AbstractTrie<V>
{
     ...
     * This is set at 32 to cover case insensitive alphabet and a few other common characters.
     */
    private static final int ROW_SIZE = 32;

    /**
     * The index lookup table, this maps a character as a byte (ISO-8859-1 or UTF8) to an index within a Trie
     * row
     */
     ...
     * This is actually a 2 dimensional array that has been flattened to achieve locality of reference. The
     * first ROW_SIZE entries are for row 0, then next ROW_SIZE entries are for row 1 etc. So in general
     * instead of using _rows[row][index], we use _rows[row*ROW_SIZE+index] to look up the next row for a
     * given character.
     *
    ...
    /**
     * A big index for each row. If a character outside of the lookup map is needed, then a big index will be
     * created for the row, with 256 entries, one for each possible byte.
     */
     ...
     * all keys stored in the Trie. The capacity needed is dependent of the shared prefixes of the keys. For
     * example, a capacity of 6 nodes is required to store keys "foo" and "bar", but a capacity of only 4 is
     * required to store "bar" and "bat".
     */
