...
/**
 * Creates asynchronously {@link SocketAddress} instances, returning them through a {@link Promise}, in order
 * to avoid blocking on DNS lookup.
 * <p>
 * {@link InetSocketAddress#InetSocketAddress(String, int)} attempts to perform a DNS resolution of the host
 * name, and this may block for several seconds. This class creates the {@link InetSocketAddress} in a
 * separate thread and provides the result through a {@link Promise}, with the possibility to specify a
...
public class SocketAddressResolver
{
    ...
    /**
     * Creates a new instance with the given executor (to perform DNS resolution in a separate thread), the
     * given scheduler (to cancel the operation if it takes too long) and the given timeout, in milliseconds.
     *
