...
/* ------------------------------------------------------------ */
/**
 * Credentials. The Credential class represents an abstract mechanism for checking authentication credentials.
 * A credential instance either represents a secret, or some data that could only be derived from knowing the
 * secret.
 * <p>
 * Often a Credential is related to a Password via a one way algorithm, so while a Password itself is a
 * Credential, a UnixCrypt or MD5 digest of a a password is only a credential that can be checked against the
 * password.
 * <p>
...
public abstract class Credential implements Serializable
{
     ...
     * Check a credential
     *
     * @param credentials The credential to check against. This may either be another Credential object, a
     * Password object or a String which is interpreted by this credential.
     * @return True if the credentials indicated that the shared secret is known to both this Credential and
    ...
    /* ------------------------------------------------------------ */
    /**
     * Get a credential from a String. If the credential String starts with a known Credential type (eg
     * "CRYPT:" or "MD5:" ) then a Credential of that type is returned. Else the credential is assumed to be a
     * Password.
     *
