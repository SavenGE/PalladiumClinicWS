...
/* ------------------------------------------------------------ */
/**
 * The Default Authenticator Factory. Uses the {@link AuthConfiguration#getAuthMethod()} to select an
 * {@link Authenticator} from: <ul>
 ...
 * wrapped with a {@link org.eclipse.jetty.security.authentication.DeferredAuthentication} instance, which is
 * used if authentication is not mandatory.
 *
 * The Authentications from the {@link org.eclipse.jetty.security.authentication.FormAuthenticator} are always
 * wrapped in a {@link org.eclipse.jetty.security.authentication.SessionAuthentication}
 * <p>
 * If a {@link LoginService} has not been set on this factory, then the service is selected by searching the
 * {@link Server#getBeans(Class)} results for a service that matches the realm name, else the first
 * LoginService found is used.
 *
 */
