 ...
 * <p>
 * This class reads an XML file conforming to the configure.dtd DTD and uses it to configure and object by
 * calling set, put or other methods on the object.</p>
 * <p>
 * The actual XML file format may be changed (eg to spring XML) by implementing the
 * {@link ConfigurationProcessorFactory} interface to be found by the {@link ServiceLoader} by using the DTD
 * and first tag element in the file. Note that DTD will be null if validation is off.</p>
 * <p>
 * The configuration can be parameterised with properties that are looked up via the Property XML element and
 * set on the configuration via the map returned from {@link #getProperties()}</p>
 * <p>
 * The configuration can create and lookup beans by ID. If multiple configurations are used, then it is good
 * practise to copy the entries from the {@link #getIdMap()} of a configuration to the next configuration so
 * that they can share an ID space for beans.</p>
 */
public class XmlConfiguration
{
     ...
     *
     * @param configuration String of XML configuration commands excluding the normal XML preamble. The String
     * should start with a "&lt;Configure ....&gt;" element.
     * @throws IOException if the configuration could not be read
     * @throws SAXException if the configuration could not be parsed
     */
     ...
     * <p>
     * A New, Get or Call XML element may have an id attribute which will cause the resulting object to be
     * placed into this map. A Ref XML element will lookup an object from this map.</p>
     * <p>
     * When chaining configuration files, it is good practise to copy the ID entries from the ID map to the
     * map of the next configuration, so that they may share an ID space
     * </p>
     *
     ...
     *
     * @param obj The object to be configured, which must be of a type or super type of the class attribute of
    ...
    /**
     * Applies the XML configuration script. If the root element of the configuration has an ID, an object is
     * looked up by ID and its type checked against the root element's type. Otherwise a new object of the
     ...
     * <p>
     * This method must be called by any {@link ConfigurationProcessor} when it creates a new instance of an
     * object before configuring it, so that a derived XmlConfiguration class may inject default values.
     *
    ...
    private static class JettyXmlConfiguration implements ConfigurationProcessor
    {
        ...
        /**
         * Recursive configuration routine. This method applies the nested Set, Put, Call, etc. elements to
         * the given object.
         *
    ...
    /**
     * Run the XML configurations as a main application. The command line is used to obtain properties files
     * (must be named '*.properties') and XmlConfiguration files.
     * <p>
     * Any property file on the command line is added to a combined Property instance that is passed to each
     * configuration file via {@link XmlConfiguration#getProperties()}.
     * <p>
     ...
     * {@link #getIdMap()}. This allows objects with IDs created in one config file to be referenced in
     * subsequent config files on the command line.
     *
