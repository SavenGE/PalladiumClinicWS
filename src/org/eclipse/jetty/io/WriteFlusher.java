 ...
 * {@link EndPoint#flush(ByteBuffer...)} until all content is written. The abstract method
 * {@link #onIncompleteFlush()} is called when not all content has been written after a call to flush and
 * should organize for the {@link #completeWrite()} method to be called when a subsequent call to flush should
...
abstract public class WriteFlusher
{
    ...
    /**
     * In FailedState no more operations are allowed. The current implementation will never recover from this
     * state.
     */
    ...
    /**
     * In CompletingState WriteFlusher is flushing buffers that have not been fully written in write(). If
     * write() didn't flush all buffers in one go, it'll switch the State to PendingState. completeWrite()
     * will then switch to this state and try to flush the remaining buffers.
     */
    ...
    /**
     * In PendingState not all buffers could be written in one go. Then write() will switch to PendingState()
     * and preserve the state by creating a new PendingState object with the given parameters.
     */
    ...
    /**
     * Abstract call to be implemented by specific WriteFlushers. It should schedule a call to
     * {@link #completeWrite()} or {@link #onFail(Throwable)} when appropriate.
     */
    abstract protected void onIncompleteFlush();

    /**
     * Tries to switch state to WRITING. If successful it writes the given buffers to the EndPoint. If state
     * transition fails it'll fail the callback.
     *
     * If not all buffers can be written in one go it creates a new <code>PendingState</code> object to
     * preserve the state and then calls {@link #onIncompleteFlush()}. The remaining buffers will be written
     * in {@link #completeWrite()}.
     *
    ...
    /**
     * Complete a write that has not completed and that called {@link #onIncompleteFlush()} to request a call
     * to this method when a call to {@link EndPoint#flush(ByteBuffer...)} is likely to be able to progress.
     *
     * It tries to switch from PENDING to COMPLETING. If state transition fails, then it does nothing as the
     * callback should have been already failed. That's because the only way to switch from PENDING outside
     * this method is {@link #onFail(Throwable)} or {@link #onClose()}
     */
