 ...
 * <P>
 * This handler does not implement the full J2EE features and is intended to be used directly when a full web
 * application is not required. If a Web application is required, then this handler should be used as part of
 * a <code>org.eclipse.jetty.webapp.WebAppContext</code>.
 * <p>
 * Unless run as part of a {@link ServletContextHandler} or derivative, the {@link #initialize()} method must
 * be called manually after start().
 */
...
public class ServletHandler extends ScopedHandler
{
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return true if ServletHandler always has a default servlet, using {@link Default404Servlet} if no
     * other default servlet is configured.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param ensureDefaultServlet true if ServletHandler always has a default servlet, using
 ...
 /*
    ...
    public void addFilterMapping(FilterMapping mapping)
    {
        if (mapping != null)
        {
            ...
            else //there are existing entries. If this is a programmatic filtermapping, it is added at the end of the list.
            ...
            else //insert non-programmatic filter mappings before any matchAfters, if any
            if (_matchAfterIndex < 0)
            {
                setFilterMappings(insertFilterMapping(mapping, mappings.length - 1, false));
            }
        ...
        else
        {
            ...
            for (String pathSpec : sms.keySet())
            {
                ...
                for (ServletMapping mapping : mappings)
                {
                    ...
                    else //already have a candidate - only accept another one if the candidate is a default
                    if (finalMapping.isDefault())
                    {
                        finalMapping = mapping;
                    }
                    else //existing candidate isn't a default, if the one we're looking at isn't a default either, then its an error
 ...
 /* ------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the maximum filter chain cache size. Filter chains are cached if {@link #isFilterChainsCached()} is
     * true. If the max cache size is greater than zero, then the cache is flushed whenever it grows to be
     * this size.
     *
