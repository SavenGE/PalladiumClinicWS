 ...
 * The default servlet.
 * <p>
 * This servlet, normally mapped to /, provides the handling for static content, OPTION and TRACE methods for
 * the context. The following initParameters are supported, these can be set either on the servlet itself or
 * as ServletContext initParameters with a prefix of org.eclipse.jetty.servlet.Default. :
 * <pre>
...
public class DefaultServlet extends HttpServlet implements ResourceFactory
{
     ...
     * In the case where the DefaultServlet is deployed on the HttpService it is likely that this method needs
     * to be overwritten to unwrap the ServletContext facade until we reach the original jetty's
     * ContextHandler.
     *
 ...
 /* (non-Javadoc)
    ...
    /* ------------------------------------------------------------ */
    /**
     * Finds a matching welcome file for the supplied {@link Resource}. This will be the first entry in the
     * list of configured {@link #_welcomes welcome files} that existing within the directory referenced by
     * the <code>Resource</code>. If the resource is not a directory, or no matching file is found, then it
     * may look for a valid servlet mapping. If there is none, then <code>null</code> is returned. The list of
     * welcome files is read from the {@link ContextHandler} for this servlet, or
