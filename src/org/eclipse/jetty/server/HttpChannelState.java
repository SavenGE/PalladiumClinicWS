...
public class HttpChannelState
{
    ...
    /**
     * The state of the servlet async API. This can lead or follow the channel dispatch state and also
     * includes reasons such as expired, dispatched or completed.
     */
    public enum Async
    {
        STARTED,
    ...
    /**
     * Signal that the HttpConnection has finished handling the request. For blocking connectors, this call
     * may block if the request has been suspended (startAsync called).
     *
     * @return next actions be handled again (eg because of a resume that happened before unhandle was called)
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Called to signal async read isReady() has returned false. This indicates that there is no content
     * available to be consumed and that once the channel enteres the ASYNC_WAIT state it will register for
     * read interest by calling {@link HttpChannel#asyncReadFillInterested()} either from this method or from
     * a subsequent call to {@link #unhandle()}.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Called to signal that content is now available to read. If the channel is in ASYNC_WAIT state and
     * unready (ie isReady() has returned false), then the state is changed to ASYNC_WOKEN and true is
     * returned.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * Called to signal that the channel is ready for a callback. This is similar to calling
     * {@link #onReadUnready()} followed by {@link #onReadPossible()}, except that as content is already
