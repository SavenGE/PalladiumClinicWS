 ...
 * HTTP Configuration.
 * <p>
 * This class is a holder of HTTP configuration for use by the {@link HttpChannel} class. Typically a
 * HTTPConfiguration instance is instantiated and passed to a {@link ConnectionFactory} that can create HTTP
 * channels (e.g. HTTP, AJP or FCGI).</p>
 * <p>
 * The configuration held by this class is not for the wire protocol, but for the interpretation and handling
 * of HTTP requests that could be transported by a variety of protocols.
 * </p>
 */
...
public class HttpConfiguration
{
    ...
    /**
     * <p>
     * An interface that allows a request object to be customized for a particular HTTP connector
     * configuration. Unlike Filters, customizer are applied before the request is submitted for processing
     * and can be specific to the connector on which the request was received.
     *
     * <p>
     * Typically Customizers perform tasks such as: <ul>
     ...
     * <li>Allow a request to be marked as secure or authenticated if those have been offloaded and
     * communicated by header, cookie or other out-of-band mechanism
     * <li>Set request attributes/fields that are determined by the connector on which the request was
     * received
     * </ul>
     */
    ...
    /**
     * <p>
     * Add a {@link Customizer} that is invoked for every request received.</p>
     * <p>
    ...
    /**
     * <p>
     * Set the {@link Customizer}s that are invoked for every request received.</p>
     * <p>
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the size of the buffer into which response content is aggregated before being sent to the client. A
     * larger buffer can improve performance by allowing a content producer to run without blocking, however
     * larger buffers consume more memory and may induce some latency before a client starts processing the
     * content.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the max size of the response content write that is copied into the aggregate buffer. Writes that
     * are smaller of this size are copied into the aggregate buffer, while writes that are larger of this
     * size will cause the aggregate buffer to be flushed and the write to be executed without being copied.
     *
     ...
     *
     * @param methods HTTP Methods of requests that can be decoded as x-www-form-urlencoded content to be made
     * available via the {@link Request#getParameter(String)} and associated APIs
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return Set of HTTP Methods of requests that can be decoded as x-www-form-urlencoded content to be made
     * available via the {@link Request#getParameter(String)} and associated APIs
     */
     ...
     *
     * @param method HTTP Method of requests that can be decoded as x-www-form-urlencoded content to be made
     * available via the {@link Request#getParameter(String)} and associated APIs
     */
     ...
     * @return True of the requests of this method type can be decoded as <code>x-www-form-urlencoded</code>
     * content to be made available via the {@link Request#getParameter(String)} and associated APIs
     */
