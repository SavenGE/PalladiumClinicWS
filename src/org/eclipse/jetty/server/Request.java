 ...
 * </p>
 * <p>
 * The standard interface of mostly getters, is extended with setters so that the request is mutable by the
 * handlers that it is passed to. This allows the request object to be as lightweight as possible and not
 * actually implement any significant behavior. For example
 * <ul>
 *
 * <li>The {@link Request#getContextPath()} method will return null, until the request has been passed to a
 * {@link ContextHandler} which matches the {@link Request#getPathInfo()} with a context path and calls
 * {@link Request#setContextPath(String)} as a result.</li>
 *
 * <li>the HTTP session methods will all return null sessions until such time as a request has been passed to
 * a {@link org.eclipse.jetty.server.session.SessionHandler} which checks for session cookies and enables the
 * ability to create new sessions.</li>
 *
 * <li>The {@link Request#getServletPath()} method will return null until the request has been passed to a
 * <code>org.eclipse.jetty.servlet.ServletHandler</code> and the pathInfo matched against the servlet URL
 * patterns and {@link Request#setServletPath(String)} called as a result.</li>
 * </ul>
 *
 * A request instance is created for each connection accepted by the server and recycled for each HTTP request
 * received via that connection. An effort is made to avoid reparsing headers and cookies that are likely to
 * be the same for requests from the same connection.
 *
 * <p>
 * The form content that a request can process is limited to protect from Denial of Service attacks. The size
 * in bytes is limited by {@link ContextHandler#getMaxFormContentSize()} or if there is no context then the
 * "org.eclipse.jetty.server.Request.maxFormContentSize" {@link Server} attribute. The number of parameters
 * keys is limited by {@link ContextHandler#getMaxFormKeys()} or if there is no context then the
...
public class Request implements HttpServletRequest
{
    ...
    /* ------------------------------------------------------------ */
    /**
     * Obtain the base {@link Request} instance of a {@link ServletRequest}, by coercion, unwrapping or
     * special attribute.
     *
     ...
     * <li>Referrer headers
     * </ul></li>
     * <li>If the request was Authenticated, an Authorization header will be set with a container generated
     * token that will result in equivalent Authorization</li>
     * <li>The query string from {@link #getQueryString()}
     * <li>The {@link #getRequestedSessionId()} value, unless at the time of the call
     * {@link #getSession(boolean)} has previously been called to create a new {@link HttpSession}, in which
     * case the new session ID will be used as the PushBuilders requested session ID.</li>
     * <li>The source of the requested session id will be the same as for this request</li>
     * <li>The builders Referer header will be set to {@link #getRequestURL()} plus any
     * {@link #getQueryString()} </li>
     ...
     * {@link PushBuilderImpl#isConditional()} header is set to true.
     * </ul>
     *
     * <p>
     * Each call to getPushBuilder() will return a new instance of a PushBuilder based off this Request. Any
     * mutations to the returned PushBuilder are not reflected on future returns.
     *
     ...
     * <dt>org.eclipse.jetty.server.HttpConnection</dt><dd>The HttpConnection or null if another transport is
     * used</dd>
     * </dl>
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return The current {@link Context context} used for this request, or <code>null</code> if
     * {@link #setContext} has not yet been called.
     */
 ...
 /*
     ...
     *
     * @return the remote {@link InetSocketAddress} for this request, or null if the request has no remote
     * (see {@link ServletRequest#getRemoteAddr()} for conditions that result in no remote address)
     */
 ...
 /*
     ...
     * name, port number, and, but it does not include a path.
     * <p>
     * Because this method returns a <code>StringBuffer</code>, not a string, you can modify the URL easily,
     * for example, to append path and query parameters.
     *
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return The resolved user Identity, which may be null if the {@link Authentication} is not
     * {@link Authentication.User} (eg. {@link Authentication.Deferred}).
     */
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the character encoding used for the query string. This call will effect the return of
     * getQueryString and getParamaters. It must be called before any getParameter methods.
     *
     * The request attribute "org.eclipse.jetty.server.server.Request.queryEncoding" may be set as an
