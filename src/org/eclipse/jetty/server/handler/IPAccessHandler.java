 ...
 * IP Access Handler
 * <p>
 * Controls access to the wrapped handler by the real remote IP. Control is provided by white/black lists that
 * include both internet addresses and URIs. This handler uses the real internet address of the connection,
 * not one reported in the forwarded for headers, as this cannot be as easily forged.
 * <p>
 ...
 * <li>Allowing a general range of IPs/URLs by specifying several general white list entries, that are then
 * further refined by several specific black list exceptions
 * </ul>
 * <p>
 * By default an empty white list is treated as match all. If there is at least one entry in the white list,
 * then a request must match a white list entry. Black list entries are always applied, so that even if an
 * entry matches the white list, a black list entry will override it.
 * <p>
 * You can change white list policy setting whiteListByPath to true. In this mode a request will be white
 * listed IF it has a matching URL in the white list, otherwise the black list applies, e.g. in default mode
 * when whiteListByPath = false and wl = "127.0.0.1|/foo", /bar request from 127.0.0.1 will be blacklisted, if
 * whiteListByPath=true then not.
 * <p>
 * Internet addresses may be specified as absolute address or as a combination of four octet wildcard
 * specifications (a.b.c.d) that are defined as follows.
 * </p>
 * <pre>
 ...
 * </pre>
 * <p>
 * Internet address specification is separated from the URI pattern using the "|" (pipe) character. URI
 * patterns follow the servlet specification for simple * prefix and suffix wild cards (e.g. /, /foo,
 * /foo/bar, /foo/bar/*, *.baz).
 * <p>
 * Earlier versions of the handler used internet address prefix wildcard specification to define a range of
 * the internet addresses (e.g. 127., 10.10., 172.16.1.). They also used the first "/" character of the URI
 * pattern to separate it from the internet address. Both of these features have been deprecated in the
 * current version.
 * <p>
 ...
 * <li>10.10.0-3,1,3,7,15|/foo/* - all requests from IPs addresses with last octet equal to 1,3,7,15 in subnet
 * 10.10.0.0/22 to URIs starting with /foo/
 * </ul>
 * <p>
 * Earlier versions of the handler used internet address prefix wildcard specification to define a range of
 * the internet addresses (e.g. 127., 10.10., 172.16.1.). They also used the first "/" character of the URI
...
public class IPAccessHandler extends HandlerWrapper
{
    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
    {
        ...
        if (channel != null)
        {
            ...
            if (endp != null)
            {
                ...
                if (address != null)
                {
                    LOG.info("==========<[client ip=<" + address.getHostString() + ">, path=<" + baseRequest.getPathInfo() + "> request]>==========");
                }
    ...
    /* ------------------------------------------------------------ */
    /**
     * Helper method to process a list of new entries and replace the content of the specified address pattern
     * map
     *
