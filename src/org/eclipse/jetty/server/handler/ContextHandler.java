 ...
 * ContextHandler.
 *
 * This handler wraps a call to handle by setting the context and servlet path, plus setting the context
 * classloader.
 *
 * <p>
 * If the context init parameter "org.eclipse.jetty.server.context.ManagedAttributes" is set to a comma
 * separated list of names, then they are treated as context attribute names, which if set as attributes are
 * passed to the servers Container so that they may be managed with JMX.
 * <p>
 * The maximum size of a form that can be processed by this context is controlled by the system properties
 * org.eclipse.jetty.server.Request.maxFormKeys and org.eclipse.jetty.server.Request.maxFormContentSize. These
 * can also be configured with {@link #setMaxFormContentSize(int)} and {@link #setMaxFormKeys(int)}
 * <p>
 * This servers executore is made available via a context attributed "org.eclipse.jetty.server.Executor".
 * <p>
 * By default, the context is created with alias checkers for {@link AllowSymLinkAliasChecker} (unix only) and
 * {@link ApproveNonExistentDirectoryAliases}. If these alias checkers are not required, then
 * {@link #clearAliasChecks()} or {@link #setAliasChecks(List)} should be called.
 */
...
public class ContextHandler extends ScopedHandler implements Attributes, Graceful
{
    ...
    /**
     * If a context attribute with this name is set, it is interpreted as a comma separated list of attribute
     * name. Any other context attributes that are set with a name from this list will result in a call to
     * {@link #setManagedAttribute(String, Object)}, which typically initiates the creation of a JMX MBean for
     * the attribute value.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set the virtual hosts for the context. Only requests that have a matching host header or fully
     * qualified URL will be passed to that context with a virtual host name. A context with no virtual host
     * names or a null virtual host name is available to all requests that are not served by a context with a
     * matching virtual host name.
     *
     * @param vhosts Array of virtual hosts that this context responds to. A null host name or null/empty
     * array means any hostname is acceptable. Host names may be String representation of IP addresses. Host
     * names may start with '*.' to wildcard one level of names. Hosts may start with '@', in which case they
     ...
     *
     * @param virtualHosts Array of virtual hosts that this context responds to. A null host name or
     * null/empty array means any hostname is acceptable. Host names may be String representation of IP
     * addresses. Host names may start with '*.' to wildcard one level of names. Host names may start with
     ...
     * @param virtualHosts Array of virtual hosts that this context responds to. A null host name or
     * null/empty array means any hostname is acceptable. Host names may be String representation of IP
     * addresses. Host names may start with '*.' to wildcard one level of names.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Get the virtual hosts for the context. Only requests that have a matching host header or fully
     * qualified URL will be passed to that context with a virtual host name. A context with no virtual host
     * names or a null virtual host name is available to all requests that are not served by a context with a
     * matching virtual host name.
     *
     * @return Array of virtual hosts that this context responds to. A null host name or empty array means any
     * hostname is acceptable. Host names may be String representation of IP addresses. Host names may start
     * with '*.' to wildcard one level of names.
     */
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set shutdown status. This field allows for graceful shutdown of a context. A started context may be put
     * into non accepting state so that existing requests can complete, but no new requests are accepted.
     *
     */
    @Override
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Extensible startContext. this method is called from {@link ContextHandler#doStart()} instead of a call
     * to super.doStart(). This allows derived classes to insert additional handling (Eg configuration) before
     * the call to super.doStart by this method will start contained handlers.
     *
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * @see org.eclipse.jetty.server.handler.ScopedHandler#doScope(java.lang.String,
    ...
    /* ------------------------------------------------------------ */
    /**
     * @see org.eclipse.jetty.server.handler.ScopedHandler#doHandle(java.lang.String,
     * org.eclipse.jetty.server.Request, javax.servlet.http.HttpServletRequest,
     * javax.servlet.http.HttpServletResponse)
     */
    @Override
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Check the target. Called by {@link #handle(String, Request, HttpServletRequest, HttpServletResponse)}
     * when a target within a context is determined. If the target is protected, 404 is returned.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param targets Array of URL prefix. Each prefix is in the form /path and will match either /path
     * exactly or /path/anything
     */
...
//            LOG.warn(this + " contextPath ends with /");
     ...
     * Set the base resource for this context.
     *
     * @param resourceBase A string representing the base resource for the context. Any string accepted by
     * {@link Resource#newResource(String)} may be passed and the call is equivalent to
     ...
     * @see
     * <a href="http://jcp.org/aboutJava/communityprocess/final/jsr154/index.html">The Servlet
     * Specification</a>
     * @see #setWelcomeFiles
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Get the character encoding for a locale. The full locale name is first looked up in the map of
     * encodings. If no encoding is found, then the locale language is looked up.
     *
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Convert URL to Resource wrapper for {@link Resource#newResource(URL)} enables extensions to provide
     * alternate resource implementations.
     *
 ...
 /*
     ...
     * Context.
     * <p>
     * A partial implementation of {@link javax.servlet.ServletContext}. A complete implementation is provided
     * by the derived {@link ContextHandler}.
     * </p>
     *
     *
     */
 ...
 /*
    ...
    /* ------------------------------------------------------------ */
    /**
     * Approve Aliases of a non existent directory. If a directory "/foobar/" does not exist, then the
     * resource is aliased to "/foobar". Accept such aliases.
     */
