 ...
 *
 * <p>
 * When {@link #handle(String, Request, HttpServletRequest, HttpServletResponse)} is called on the first
 * ScopedHandler in a chain of HandlerWrappers, the
 * {@link #doScope(String, Request, HttpServletRequest, HttpServletResponse)} method is called on all
 * contained ScopedHandlers, before the
 * {@link #doHandle(String, Request, HttpServletRequest, HttpServletResponse)} method is called on all
 * contained handlers.</p>
 *
 * <p>
 * For example if Scoped handlers A, B &amp; C were chained together, then the calling order would be:</p>
 * <pre>
 * A.handle(...)
 *   A.doScope(...)
 *     B.doScope(...)
 ...
 *
 * <p>
 * If non scoped handler X was in the chained A, B, X &amp; C, then the calling order would be:</p>
 * <pre>
 * A.handle(...)
 *   A.doScope(...)
 *     B.doScope(...)
