...
/**
 * <p>
 * {@link HttpOutput} implements {@link ServletOutputStream} as required by the Servlet specification.</p>
 * <p>
 * {@link HttpOutput} buffers content written by the application until a further write will overflow the
 * buffer, at which point it triggers a commit of the response.</p>
 * <p>
 * {@link HttpOutput} can be closed and reopened, to allow requests included via
 * {@link RequestDispatcher#include(ServletRequest, ServletResponse)} to close the stream, to be reopened
 * after the inclusion ends.</p>
 */
public class HttpOutput extends ServletOutputStream implements Runnable
{
    ...
    /**
     * Bytes written via the write API (excludes bytes written via sendContent). Used to autocommit once
    ...
    /**
     * Called to indicate that the last write has been performed. It updates the state and performs cleanup
     * operations.
     */
    void closed()
    {
     ...
     * {@link HttpChannel}. A non direct buffer of size {@link HttpOutput#getBufferSize()} is used. This
     * callback is passed to the {@link HttpChannel#write(ByteBuffer, boolean, Callback)} to be notified as
     * each buffer is written and only once all the input is consumed will the wrapped
     ...
     * {@link HttpChannel}. A {@link ByteBuffer} of size {@link HttpOutput#getBufferSize()} is used that will
     * be direct if {@link HttpChannel#useDirectBuffers()} is true. This callback is passed to the
     * {@link HttpChannel#write(ByteBuffer, boolean, Callback)} to be notified as each buffer is written and
     * only once all the input is consumed will the wrapped {@link Callback#succeeded()} method be called.
     */
