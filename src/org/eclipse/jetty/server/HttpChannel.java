...
/**
 * HttpChannel represents a single endpoint for HTTP semantic processing. The HttpChannel is both a
 * HttpParser.RequestHandler, where it passively receives events from an incoming HTTP request, and a
 * Runnable, where it actively takes control of the request/response life cycle and calls the application
 * (perhaps suspending and resuming with multiple calls to run). The HttpChannel signals the switch from
 * passive mode to active mode by returning true to one of the HttpParser.RequestHandler callbacks. The
 * completion of the active phase is signalled by a call to HttpTransport.completed().
 *
 */
public class HttpChannel implements Runnable, HttpOutput.Interceptor
{
     ...
     * <p>
     * This is implemented as a call to {@link EndPoint#getIdleTimeout()}, but may be overridden by channels
     * that have timeouts different from their connections.
     *
     ...
     * <p>
     * This is implemented as a call to {@link EndPoint#setIdleTimeout(long)}, but may be overridden by
     * channels that have timeouts different from their connections.
     *
    ...
    /**
     * If the associated response has the Expect header set to 100 Continue, then accessing the input stream
     * indicates that the handler/servlet is ready for the request body and thus a 100 Continue response is
     * sent.
     *
    ...
    public boolean handle()
    {
        ...
        try
        {
            ...
            loop:
            while (action.ordinal() < HttpChannelState.Action.WAIT.ordinal() && !getServer().isStopped())
            {
                boolean error = false;
                try
                {
    ...
    /**
     * <p>
     * Sends an error 500, performing a special logic to detect whether the request is suspended, to avoid
     * concurrent writes from the application.</p>
     * <p>
     * It may happen that the application suspends, and then throws an exception, while an application spawned
     * thread writes the response content; in such case, we attempt to commit the error directly bypassing the
     * {@link ErrorHandler} mechanisms and the response OutputStream.</p>
     *
    ...
    /**
     * @return true if the HttpChannel can efficiently use direct buffer (typically this means it is not over
     * SSL or a multiplexed protocol)
     */
