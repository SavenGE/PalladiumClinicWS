 ...
 * every time the session is accessed. Rather, we write it out every so often. The frequency is controlled by
 * the saveIntervalSec field.
 */
public class JDBCSessionManager extends AbstractSessionManager
{
    ...
    public class Session extends MemSession
    {
        ...
        /**
         * Entry to session. Called by SessionHandler on inbound request and the session already exists in
         * this node's memory.
         *
     ...
     *
     * This is an optimization that prevents the database from being overloaded when a session is accessed
     * very frequently.
     *
     * On session exit, if the session attributes have NOT changed, the time at which we last saved the
     * accessed time is compared to the current accessed time. If the interval is at least saveIntervalSecs,
     * then the access time will be persisted to the database.
     *
     * If any session attribute does change, then the attributes and the accessed time are persisted.
     *
    ...
    /**
     * A method that can be implemented in subclasses to support distributed caching of sessions. This method
     * will be called whenever the session is written to the database because the session data has changed.
     *
     * This could be used eg with a JMS backplane to notify nodes that the session has changed and to delete
     * the session from the node's cache, and re-read it from the database.
     *
     ...
     *
     * Load the session by id AND context path from the database. Multiple contexts may share the same session
     * id (due to dispatching) but they CANNOT share the same contents.
     *
     * Check if last node id is my node id, if so, then the session we have in memory cannot be stale. If
     * another node used the session last, then we need to refresh from the db.
     *
     * NOTE: this method will go to the database, so if you only want to check for the existence of a Session
     * in memory, use _sessions.get(id) instead.
     *
     ...
     *
     * @see
     * org.eclipse.jetty.server.session.AbstractSessionManager#addSession(org.eclipse.jetty.server.session.AbstractSession)
     */
    @Override
     ...
     *
     * @see
     * org.eclipse.jetty.server.session.AbstractSessionManager#newSession(javax.servlet.http.HttpServletRequest)
     */
    @Override
        ...
        try
        {
            ...
            while (itor.hasNext())
            {
                ...
                if (session == null)
                {
                    if (LOG.isDebugEnabled())
                    {
                        LOG.debug("Force loading session id " + sessionId);
                    }
