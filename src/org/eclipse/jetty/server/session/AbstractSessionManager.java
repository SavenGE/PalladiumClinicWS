 ...
 * <p>
 * The partial implementation of SessionManager interface provides the majority of the handling required to
 * implement a SessionManager. Concrete implementations of SessionManager based on AbstractSessionManager need
 * only implement the newSession method to return a specialized version of the Session inner class that
...
public abstract class AbstractSessionManager extends ContainerLifeCycle implements SessionManager
{
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return same as SessionCookieConfig.getSecure(). If true, session cookies are ALWAYS marked as secure.
     * If false, a session cookie is ONLY marked as secure if _secureRequestOnly == true and it is a HTTPS
     * request.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * HTTPS request. Can be overridden by setting SessionCookieConfig.setSecure(true), in which case the
     * session cookie will be marked as secure on both HTTPS and HTTP.
     *
     ...
     * <li>SessionCookieConfig.setSecure == false &amp;&amp; _secureRequestOnly==true &amp;&amp; request is
     * HTTPS</li>
     * </ol>
     * According to SessionCookieConfig javadoc, case 1 can be used when: "... even though the request that
     * initiated the session came over HTTP, is to support a topology where the web container is front-ended
     * by an SSL offloading load balancer. In this case, the traffic between the client and the load balancer
     * will be over HTTPS, whereas the traffic between the load balancer and the web container will be over
     * HTTP."
     * <p>
     * For case 2, you can use _secureRequestOnly to determine if you want the Servlet Spec 3.0 default
     * behavior when SessionCookieConfig.setSecure==false, which is:
     * <cite>
     * "they shall be marked as secure only if the request that initiated the corresponding session was also
     * secure"
     * </cite>
     * <p>
     * The default for _secureRequestOnly is true, which gives the above behavior. If you set it to false,
     * then a session cookie is NEVER marked as secure, even if the initiating request was secure.
     *
     * @see org.eclipse.jetty.server.SessionManager#getSessionCookie(javax.servlet.http.HttpSession,
    ...
    /* ------------------------------------------------------------ */
    /**
     * Add the session Registers the session with this manager and registers the session ID with the
     * sessionIDManager;
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return true if the cluster node id (worker id) is returned as part of the session id by
     * {@link HttpSession#getId()}. Default is false.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param nodeIdInSessionId true if the cluster node id (worker id) will be returned as part of the
     * session id by {@link HttpSession#getId()}. Default is false.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * Tell the HttpSessionIdListeners the id changed. NOTE: this method must be called LAST in subclass
     * overrides, after the session has been updated with the new id.
     *
     * @see org.eclipse.jetty.server.SessionManager#renewSessionId(java.lang.String, java.lang.String,
 ...
 /* ------------------------------------------------------------ */
    /**
     * Interface that any session wrapper should implement so that SessionManager may access the Jetty session
     * implementation.
     *
     */
