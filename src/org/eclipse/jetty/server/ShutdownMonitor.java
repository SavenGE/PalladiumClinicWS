 ...
 * <p>
 * This thread listens on the host/port specified by the STOP.HOST/STOP.PORT system parameter (defaults to
 * 127.0.0.1/-1 for not listening) for request authenticated with the key given by the STOP.KEY system
 * parameter (defaults to "eclipse") for admin requests.
 * <p>
...
public class ShutdownMonitor
{
     ...
     *
     * Thread for listening to STOP.PORT for command to stop Jetty. If ShowndownMonitor.exitVm is true, then
     * Sytem.exit will also be called after the stop.
     *
     */
     ...
     * <p>
     * <code>STOP.PORT</code> = the port to listen on (empty, null, or values less than 0 disable the stop
     * ability)<br>
     * <code>STOP.KEY</code> = the magic key/passphrase to allow the stop (defaults to "eclipse")<br>
     * <p>
     * Note: server socket will only listen on localhost, and a successful stop will issue a System.exit()
     * call.
     */
