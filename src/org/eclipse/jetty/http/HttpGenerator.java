 ...
 * <p>
 * If the system property "org.eclipse.jetty.http.HttpGenerator.STRICT" is set to true, then the generator
 * will strictly pass on the exact strings received from methods and header fields. Otherwise a fast case
 * insensitive string lookup is used that may alter the case and white space of some methods/headers
 */
public class HttpGenerator
{
    ...
    private final static HttpHeaderValue[] CLOSE =
    {
    ...
    public final static MetaData.Response RESPONSE_500_INFO
        = new MetaData.Response(HttpVersion.HTTP_1_1, HttpStatus.INTERNAL_SERVER_ERROR_500, null, new HttpFields()
        {
            {
            ...
            for (HttpField field : _info.getFields())
            {
                ...
                switch (h == null ? HttpHeader.UNKNOWN : h)
                {
                    ...
                    case CONNECTION:
                    {
                        ...
                        HttpHeaderValue[] values = HttpHeaderValue.CLOSE.is(field.getValue()) ? CLOSE : new HttpHeaderValue[]
                        {
                            HttpHeaderValue.CACHE.get(field.getValue())
                        };
                        String[] split = null;
