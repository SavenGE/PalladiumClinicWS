 ...
 * {@link #parseNext(ByteBuffer)} method. The parsed elements of the HTTP message are passed as event calls to
 * the {@link HttpHandler} instance the parser is constructed with. If the passed handler is a
 * {@link RequestHandler} then server side parsing is performed and if it is a {@link ResponseHandler}, then
 * client side parsing is done.
 * </p>
 * <p>
 * The contract of the {@link HttpHandler} API is that if a call returns true then the call to
 * {@link #parseNext(ByteBuffer)} will return as soon as possible also with a true response. Typically this
 * indicates that the parsing has reached a stage where the caller should process the events accumulated by
 * the handler. It is the preferred calling style that handling such as calling a servlet to process a
 * request, should be done after a true return from {@link #parseNext(ByteBuffer)} rather than from within the
 * scope of a call like {@link RequestHandler#messageComplete()}
 * </p>
 * <p>
 * For performance, the parse is heavily dependent on the {@link Trie#getBest(ByteBuffer, int, int)} method to
 * look ahead in a single pass for both the structure ( : and CRLF ) and semantic (which header and value) of
 * a header. Specifically the static {@link HttpHeader#CACHE} is used to lookup common combinations of headers
 * and values (eg. "Connection: close"), or just header names (eg. "Connection:" ). For headers who's value is
 * not known statically (eg. Host, COOKIE) then a per parser dynamic Trie of {@link HttpFields} from previous
 * parsed messages is used to help the parsing of subsequent messages.
 * </p>
 * <p>
 * If the system property "org.eclipse.jetty.http.HttpParser.STRICT" is set to true, then the parser will
 * strictly pass on the exact strings received for methods and header fields. Otherwise a fast case
 * insensitive string lookup is used that may alter the case of the method and/or headers
 * </p>
 * <p>
...
public class HttpParser
{
     ...
     * <li>Most common headers with null values so that a lookup will at least determine the header name even
     * if the name:value combination is not cached
     * </ul>
     */
...
//        System.err.println("XXX Input XXX-- "+input);
        ...
        {
            case ILLEGAL:
                //System.err.println("IllegalCharacterException 1");
            ...
            switch (_state)
            {
                case METHOD:
                    ...
                    else if (ch < SPACE)
                    {
                        ...
                        {
                            //System.err.println("IllegalCharacterException 2");
                            throw new IllegalCharacterException(_state, ch, buffer);
                        }
                    }
                    else
                ...
                case RESPONSE_VERSION:
                    ...
                    {
                        //System.err.println("IllegalCharacterException 3");
                        throw new IllegalCharacterException(_state, ch, buffer);
                    }
                    else
                    {
        ...
        while (_state.ordinal() < State.CONTENT.ordinal() && buffer.hasRemaining() && !handle)
        {
            ...
            switch (_state)
            {
                ...
                case HEADER_IN_NAME:
                    ...
                    //System.err.println("IllegalCharacterException 4");
                    throw new IllegalCharacterException(_state, ch, buffer);
                ...
                case HEADER_VALUE:
                    ...
                    //System.err.println("IllegalCharacterException 5");
                    throw new IllegalCharacterException(_state, ch, buffer);
                ...
                case HEADER_IN_VALUE:
                    ...
                    //System.err.println("IllegalCharacterException 6");
                    throw new IllegalCharacterException(_state, ch, buffer);
        ...
        while (_state.ordinal() < State.END.ordinal() && remaining > 0)
        {
            switch (_state)
            {
                ...
                case CHUNK_END:
                {
                    ...
                    //System.err.println("IllegalCharacterException 7");
                    throw new IllegalCharacterException(_state, ch, buffer);
                }
 ...
 /* ------------------------------------------------------------------------------- */
    public interface RequestHandler extends HttpHandler
    {
         ...
         * @param uri The raw bytes of the URI. These are copied into a ByteBuffer that will not be changed
         * until this parser is reset and reused.
