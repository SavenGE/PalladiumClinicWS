...
public class HttpServer extends com.sun.net.httpserver.HttpServer
{
    ...
    private Map<String, HttpContext> _contexts = new HashMap<>();

    private Map<String, Connector> _connectors = new HashMap<>();

    public HttpServer(Server server, boolean shared)
    {
        this(server, shared, new HttpConfiguration());
    }

    public HttpServer(Server server, boolean shared, HttpConfiguration configuration)
    {
        this._server = server;
    ...
    {
        for (Map.Entry<String, HttpContext> stringJettyHttpContextEntry : _contexts.entrySet())
        {
            HttpContext context = stringJettyHttpContextEntry.getValue();
    ...
    @Override
    public com.sun.net.httpserver.HttpContext createContext(String path, HttpHandler httpHandler)
    {
        checkIfContextIsFree(path);

        HttpContext context = new HttpContext(this, path, httpHandler);
    ...
    {
        HttpContext context = _contexts.remove(path);
        if (context == null)
        {
            return;
        }
